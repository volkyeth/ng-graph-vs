...
export const graphIdAtom = atom(
  (get) => {
    const pathName = get(locationAtom).pathname;
    if (!pathName) return undefined;
    if (pathName === "/") return undefined;
    return decodeURI(pathName.substring(1));
  },
...
export const graphStateAtom = atom(
  (get) => {
...
export const helpDialogOpenAtom = atomWithStorage("helpDialogOpen", true);
